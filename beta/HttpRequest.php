...
class HttpReq
{
    ...
    public function __construct(string $url, bool $post = FALSE, string $postData = "", bool $proxy = FALSE, bool $referer = FALSE, string $refererData = "", bool $header = FALSE, array $headerData = [], bool $encoding = FALSE, string $encodingData = "")
    {
        $this->url = $url;
        $this->post = $post;
    ...
    public function request()
    {
        ...
        /* if ((empty(curl_getinfo($this->curl, CURLINFO_HTTP_CODE))) OR (curl_getinfo($this->curl, CURLINFO_HTTP_CODE) != 0) OR (curl_getinfo($this->curl, CURLINFO_HTTP_CODE) != 404))
